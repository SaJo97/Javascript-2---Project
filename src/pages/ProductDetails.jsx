        ...
        <div className="image-slider">
          ...
          <div className="thumbnails">
            {images.map((src, index) => (
              <img key={index} src={src} alt={`Thumbnail ${index + 1}`} onClick={() => setMainImage(src)} />
            ))}
          </div>
        </div>
        <div className="details">
          <h1>{product.name}</h1>
          <div className="price"><p>{product.price}kr</p></div>
          <div className="description">
            <p>{product.description}</p>
          </div>
          <div className="add-to-cart2">
            <button className="cart-btn2" onClick={handleAddToCart}>Add to Cart</button>
          </div>
        </div>
      </div>
  )
